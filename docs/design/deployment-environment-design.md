...
| `CHAT_PROVIDER` | no | 予約支援チャットのAIプロバイダ | `mock` / `gemini` / `groq` / `ollama` |
| `GEMINI_API_KEY` | no | Gemini API を使う場合のAPIキー | `CHAT_PROVIDER=gemini` のとき使用 |
| `GEMINI_MODEL` | no | Gemini API のモデル名 | 例: `gemini-flash-latest` |
| `GROQ_API_KEY` | no | Groq API を使う場合のAPIキー | `CHAT_PROVIDER=groq` のとき使用 |
| `GROQ_MODEL` | no | Groq API のモデル名 | 例: `llama-3.1-8b-instant` |
| `OLLAMA_BASE_URL` | no | ローカルOllamaのURL | ローカルデモ用 |
| `OLLAMA_MODEL` | no | Ollamaのモデル名 | ローカルデモ用 |
