...
5. `guests` を作成する。
6. `generateReservationNumber`（`src/lib/reservations/reservationNumber.ts`）で `HRS-YYYYMMDD-NNNN` 形式の予約番号を発行する。
...
5. `evaluateCheckIn`（`src/lib/reservations/checkIn.ts`）で予約状態・チェックイン可能日を確認する。
...
4. `classifyMissingStay`（`src/lib/stays/checkOut.ts`）で宿泊が存在しない原因を分類する（未チェックイン vs チェックアウト済み）。
