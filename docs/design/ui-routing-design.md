...
- 予約支援チャットは主要業務画面ではなく補助機能として扱い、グローバルナビではなく右下固定の導線から開く。PCでは小窓ウィジェット、スマートフォンでは専用画面 `/chat` へ遷移する。
...
| 返してよい情報 | 返さない情報 |
| --- | --- |
| 予約番号、予約状態、部屋タイプ名、宿泊予定日、泊数、宿泊人数、割当済みの部屋番号、支払結果の概要、照合済み予約の代表者名・メールアドレス・電話番号 | `guestId`, `reservationId`, `stayId`, DB内部ID、未照合の連絡先、作成日時、更新日時、内部メモ、サーバーログ、SQLエラー |

利用者がフォームに入力した氏名や連絡先は、その利用者自身のブラウザ上では見える。予約番号と氏名で本人照合できた予約詳細では、登録済みの代表者名・メールアドレス・電話番号を確認用に表示してよい。設計上避けるべきなのは、照合前の連絡先や内部IDをAPIレスポンス、URL、HTML、ログへ不要に残すことである。
...
| `/chat` | CSR + Route Handler | 入力中の会話UIはCSRで扱い、空室検索などの読み取り処理はサーバー側で実行するため |
...
| `/chat` | 予約支援チャット | スマートフォン向けの専用画面として表示し、空室・部屋タイプ・キャンセルポリシー・各種手続きの案内を読み取り専用で行う |
| `/reservations/[reservationNumber]` | 予約結果・予約詳細 | 予約作成後または予約情報照合後に予約内容を表示する |
| `/reservations/lookup` | 予約確認 | 予約番号と氏名（姓・名）を入力し予約内容を確認する |
| `/check-in` | チェックイン | 予約番号と氏名（姓・名）を入力しチェックインする |
| `/check-out` | チェックアウト | 部屋番号を入力し、料金確認後に支払い情報を入力してチェックアウトする |
| `/reservations/cancel` | 予約キャンセル | 予約番号と氏名（姓・名）を入力し、予約内容確認後にキャンセルする |

`/reservations/[reservationNumber]` は直接アクセス時に氏名による本人照合を完了できないため、予約詳細を表示する前に `/reservations/lookup` へ誘導する。

## 画面と API の対応

| 画面 | 呼び出す API | 備考 |
| --- | --- | --- |
| `/reservations/new` | `GET /api/room-types`, `GET /api/availability/calendar`, `GET /api/availability`, `POST /api/reservations` | 空室カレンダーで日別空室を表示し、予約作成後に予約番号を表示する |
| `/chat` | `POST /api/chat` | 予約作成やキャンセル確定は実行せず、必要な画面へ誘導する。最新メッセージへ自動スクロールし、送信制限時もチャット内で案内する |
| `/reservations/lookup` | `GET /api/reservations/{reservationNumber}` | 予約番号と氏名（姓・名）で照会する |
| `/check-in` | `POST /api/reservations/{reservationNumber}/check-in` | 予約番号と氏名（姓・名）を照合し、チェックインを実行する |
...
flowchart TD
    ...
    roomTypes["部屋タイプを取得"]
    input["宿泊条件・利用者情報を入力"]
    calendar["月別空室カレンダーを確認"]
    availability["空室と概算料金を確認"]
    roomDetail["客室タイプの詳細を確認"]
    submit["予約を確定"]
    result["予約番号と予約内容を表示"]

    start --> roomTypes --> input --> calendar --> availability --> roomDetail --> submit --> result
```

予約作成後は、予約番号と宿泊代表者の氏名（姓・名）が今後の予約照会に必要であることを画面上で明確に表示する。
...
チェックインは予約番号と氏名（姓・名）を照合したうえで直接実行する。チェックインはチェックイン予定日当日のみ実行でき、完了時に部屋番号と宿泊内容を表示する。
