...
HRS では、予約作成、チェックイン、チェックアウトで複数テーブルを一体で更新する。これらは途中で失敗すると予約状態や宿泊情報が不整合になるため、アプリケーション層のユースケースサービスでトランザクション境界を持つ。
...
- 予約作成、チェックイン、チェックアウトは必ず `Prisma.$transaction` 内で実行する。
...
| チェックイン | 本人確認、予約状態確認、部屋割当、宿泊作成、予約状態更新 | 二重チェックイン、同じ部屋の二重割当 |
| チェックアウト | 本人確認、宿泊確認、料金作成、支払い作成、予約状態更新 | 二重チェックアウト、支払金額不一致 |
