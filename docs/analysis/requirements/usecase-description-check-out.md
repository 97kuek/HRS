...
| 事後条件 | 宿泊が終了し，HRSにチェックアウト日時と支払いが記録され，予約の状態がチェックアウト済みに更新され，利用者に領収メールが送信されている |
| 失敗時の事後条件 | 宿泊は終了せず，支払いは記録されない。利用者にチェックアウトできない理由が示されている |

## 基本系列

1. 利用者が部屋番号を入力する。
2. HRSが部屋番号に対応する滞在中の宿泊（チェックアウト日時が未記録のもの）を特定する。
...
> **注**: 実際の決済処理はシステム外で行うため，現実装では提示された請求額と支払額の不一致を支払い失敗として扱う。
