...
> ※ `usecase-diagram.asta` と `ユースケース図.png` は別途 astah で作成して追加する．
...
確定スコープとして，対応する記述担当 Issue があるユースケースを4つ挙げる．
...
- **予約を取消する**: ドメイン分析の予約状態に「取消済」があり，技術選定で検討中の API にも予約取消が含まれる．しかし記述を担当する Issue が存在しない．独立ユースケースとして追加するか，今回のスコープ外とするかを決める．
