  ...
  useEffect(() => {
    ...
    async function loadCalendar() {
      ...
      try {
        ...
        const data = (await res.json()) as AvailabilityCalendarResponse | ApiErrorResponse;
        if (cancelled) return;
        if (!res.ok) {
          setCalendarDays([]);
          setCalendarError((data as ApiErrorResponse).error.message);
          return;
        }
...
function Step4({
  ...
  async function confirm() {
    ...
    try {
      ...
      const data = (await res.json()) as { reservation: ReservationResult } | ApiErrorResponse;
      if (!res.ok) {
